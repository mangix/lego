    ...
    this._chain.push(function () {
        ...
        args.forEach(function (ModuleBrick) {
            ...
            tasks[ModuleBrick.Name] = function (cb) {
                brick.handle(self.params, function (status, data, html) {
        ...
        async.parallel(tasks, function (err, results) {
            ...
            Object.keys(results).forEach(function (brickName) {
                ...
                if (data) {
                    self.params[brickName] = data.data;
                    self.params[brickName + "View"] = data.html;
                } else {
                    self.params[brickName] = null;
                }
            });
            ...
            self.propertyQueue.forEach(function (trans) {
                if (trans.brick && trans.brick.Name && trans.brick.Name in results) {
                    transProperty(results[trans.brick.Name] ? results[trans.brick.Name].data : null, self.params, trans.property, trans.toProperty);
                }
            });
